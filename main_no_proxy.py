...
def get_clip_views(clip_id):
    """Fetch current clip views from Kick.com API (no proxy)"""
...
def simulate_view(clip_url, channel_name, clip_id, bot_id, views_per_bot):
    """Simulate views for a clip using a single bot (no proxy)"""
    ...
    for view_num in range(1, views_per_bot + 1):
        try:
            ...
            # Create session with random user agent (no proxy)
            session = requests.Session(impersonate="firefox135")
            session.timeout = 15
            ...
            # Random delay between requests (5-15 seconds - longer without proxy)
...
def display_dashboard(clip_id, target_views, num_bots):
    """Display real-time dashboard (no proxy info)"""
    ...
    while True:
        try:
            ...
            current_views = get_clip_views(clip_id)
            ...
            # Dashboard display
            print("🚀 KICK.COM CLIP VIEW BOT (NO PROXY) 🚀")
            print("=" * 60)
            ...
            print("\n⚠️  NOTE: Running without proxies - all requests from your IP")
...
def main():
    ...
    # Get number of bots from user (no proxy limit)
    try:
        num_bots = int(input("🤖 Number of bots (recommended: 5-20): ") or "10")
    except ValueError:
        num_bots = 10
    ...
    print("⚠️  WARNING: All requests will come from your IP address!")
    print("⚠️  Consider using fewer bots to avoid rate limiting.")
    ...
    initial_views = get_clip_views(clip_id)
    ...
    for i in range(num_bots):
        thread = threading.Thread(
            ...
            args=(clip_url, channel_name, clip_id, i+1, views_per_bot),
            daemon=True
        )
        threads.append(thread)
        thread.start()
        
        # Longer delay between starting threads to avoid overwhelming
        time.sleep(0.5)
    ...
    try:
        display_dashboard(clip_id, target_views, num_bots)
    except KeyboardInterrupt:
        print("\n🛑 STOPPING BOTS...")
